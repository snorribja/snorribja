"""
Render "SB" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
# blank rows above and below the art. 3 makes a 486x372 panel, which renders at
# 490 wide beside the 370-wide portrait and lands within 10px of its height.
...
FONT_PATH = os.environ.get("WORDMARK_FONT", FUTURA if os.path.exists(FUTURA) else "DejaVuSans-Bold.ttf")
FONT_INDEX = int(os.environ.get("WORDMARK_FONT_INDEX", 2 if FONT_PATH == FUTURA else 0))
# Initials keep each glyph wide enough to read as characters rather than dither.
TEXT = os.environ.get("WORDMARK_TEXT", "SB")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
        f'font-size="11.5" text-anchor="middle">SB / ASCII OBJECT · ICELAND / NEW YORK</text>')
